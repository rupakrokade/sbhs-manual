...
function [thetaN_oe,covN_oe,nvar] = oe(zd,nb,nf,nk)

[thetaN,covfN,nvar] = arxc(zd,nf,nb,nk);
[r1,c1] = size(thetaN);
yt = zd(:,1);
m=50;
  if nf==0
    ...
    for k=1:m
      ...
      [thetaNf,covf_a,nvar] = arxc(zdf,nf,nb,nk)
      thetaN = thetaNf;
      a1 = thetaN(1:nf);
      b = (norm(a-a1))/norm(a1);
        if b<0.005
...
function [thetaN_arx,covt_arx,nvar] = arxc(data,na,nb,nk)
az = max(na,nb+nk-1);
zer = zeros(az,1);
zd = data;
// Zeros appended
